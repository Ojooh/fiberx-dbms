...
const g = async () => {
    ...
    await dbms.initializeDBMS("fibase", "teryuiotp");

    // Try creating/deleting schema
    const model_name = "Role"
    const table_name = "fibase_roles"
    const datasource = "mysql_db"
    const columns = {
        ...
        updated_at: { type: DataTypes.DATE(), default: null, on_update: "CURRENT_TIMESTAMP" },
    };
    const primary_key = "id"
    const  indexes = [ { name: 'idx_fibase_role_symbol', fields: ['symbol'] } ];
