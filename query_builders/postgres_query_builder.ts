...
class PostgresQueryBuilder extends BaseQueryBuilder {
    ...
    beginTransaction(): string { return 'BEGIN'; }
    ...
    createTable(table_schema: TableSchemaInterface): string {
        ...
        const columns_def = Object.entries(columns).map(([col, options]) => {
            ...
            let default_value = ""
            if (options.default === "CURRENT_TIMESTAMP") { default_value = " DEFAULT NOW()"; }
    ...
    addColumn(table_name: string, column_name: string, column_def:TableColumnInterface, position: ColumnPositionType):string {
        const type          = mapToPostgresType(column_def.type);
        ...
        // Add DEFAULT value if present
        let default_val = ""
        if (column_def.default === "CURRENT_TIMESTAMP") { default_val = " DEFAULT NOW()"; }
        ...
        if (column_def.auto_increment) {
            if (type === "INT") { column_type = "SERIAL";} 
            
            else if (type === "BIGINT") { column_type = "BIGSERIAL"; }
            
            else { throw new Error(`Auto-increment is not supported for type ${type} in PostgreSQL`); }
    ...
    dropColumn(table_name: string, column_name: string):string { return `ALTER TABLE "${this.escapeField(table_name)}" DROP COLUMN "${this.escapeField(column_name)}"`.trim(); }

    dropIndex(table_name: string, index_name: string):string { return `DROP INDEX IF EXISTS "${this.escapeField(index_name)}" ON "${this.escapeField(table_name)}"`; }
