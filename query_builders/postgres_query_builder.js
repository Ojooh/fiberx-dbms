...
class PostgresQueryBuilder {
    ...
    #formatOptions = (options = null) => {
        ...
        const { limit, offset, order, distinct } = options;
    ...
    #formatColumnDefinition = (col, options) => {
        const sql_type      = mapToPostgresType(options.type);
        let col_def         = `${this.query_util.escapeField(col)} ${sql_type}`;

        // Add SERIAL/BIGSERIAL for auto-incrementing primary keys
        if (options.auto_increment && sql_type === 'INT') { col_def = `${this.escapeField(col)} SERIAL`; }
        
        else if (options.auto_increment && sql_type === 'BIGINT') { col_def = `${this.escapeField(col)} BIGSERIAL`; }

        if (options.unique) { col_def += ' UNIQUE'; }

        if (options.default !== undefined) {
            if (options.default === 'CURRENT_TIMESTAMP') { col_def += ' DEFAULT NOW()'; }
        ...
        if (options.references) {
            ...
            col_def     += ` REFERENCES "${ref.table}"("${ref.column}")`;
    ...
    select = (table_name, fields, where = {}, options = {}) => {
        ...
        const sql = `SELECT${this.#formatOptions(options).includes('DISTINCT') ? ' DISTINCT' : ''} ${full_fields} FROM "${table_name}" ${join_clause} ${this.#formatWhereClause(where)} ${this.#formatOptions(options)}`;
        return sql.trim();
    }
    ...
    selectCount = (table_name, where = {}, options = {}) => {
        const { include = [] } = options;
        const { joins } = this.#formatIncludes(include, table_name);
        return `SELECT COUNT(*) as count FROM "${table_name}" ${joins.join(' ')} ${this.#formatWhereClause(where)} ${this.#formatOptions(options)}`.trim();
    }
    ...
    delete = (table_name, where) => { return `DELETE FROM "${table_name}" ${this.#formatWhereClause(where)}`.trim(); }
    ...
    createTable = (schema) => {
        ...
        return `CREATE TABLE "${table_name}" (${cols_def}${pk})`;
    }

    // Method to add a column
    addColumn = (table_name, column_name, def) => {
        const col_def = this.#formatColumnDefinition(column_name, def);
        return `ALTER TABLE "${table_name}" ADD COLUMN ${col_def}`.trim();
    }
    ...
    createIndex = (table_name, index_fields, unique) => {
        ...
        return `CREATE ${unique ? 'UNIQUE ' : ''}INDEX "${idxName}" ON "${table_name}" (${fields})`;
    }

    // Method to get drop column query
    dropColumn = (table_name, column_name) => { return `ALTER TABLE "${table_name}" DROP COLUMN "${column_name}"`; }

    // Method to get deop index query
    dropIndex = (table_name, index_name) => { return `DROP INDEX "${index_name}"`; }

    // Method to get drop table query
    dropTable = (table_name) => { return `DROP TABLE IF EXISTS "${table_name}"`; }
}
